# Reference: https://github.com/fastai/fastai/blob/aa58b1316ad8e7a5fa2e434e15e5fe6df4f4db56/fastai/callback/schedule.py
import tempfile
    ...
    def plot_lrs(
        ...
        if suggestion:
            try:
                ...
                return
            print(f"Min numerical gradient (min_lr): {lrs[mg]:.2E}")
            ax.plot(lrs[mg], losses[mg], markersize=10, marker="o", color="red")
            self.min_grad_lr = lrs[mg]
            self.min_grad_lr_idx = losses[mg]
            ml = np.argmin(losses)
            print(f"Min loss divided by 10 (max_lr): {lrs[ml]/10:.2E}")

    def slide(
        self, lr_diff: int = 15, thresh: float = 0.005, adjust_value: float = 1.0,
    ):
